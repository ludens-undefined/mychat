...
type CourseAudio struct {
	...
	Name            string  `orm:"column:name;size:50"` //音频名称
	AudioName       string  `orm:"column:audio_name;size:255"` //音频名称
	Audio           string  `orm:"column:audio;size:255"` //音频路径
	Img             string  `orm:"column:img;size:255"` //音频封面
	IsShow          uint8   `orm:"column:is_show"` //1购买前查看完整商品详情 2购买前仅查看介绍内容
	Intro           string  `orm:"column:intro;size:100"` //商品介绍内容
...
func (t *CourseAudio) TableName() string {
	return "goouc_xmf_course_audio"
}

func init() {
	orm.RegisterModel(new(CourseAudio))
}

// AddCourseAudio insert a new CourseAudio into database and returns
// last inserted Id on success.
func AddCourseAudio(m *CourseAudio) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetCourseAudioById retrieves CourseAudio by Id. Returns error if
// Id doesn't exist
func GetCourseAudioById(id int) (v *CourseAudio, err error) {
	o := orm.NewOrm()
	v = &CourseAudio{Id: id}
...
// GetAllCourseAudio retrieves all CourseAudio matches certain condition. Returns empty list if
// no records exist
func GetAllCourseAudio(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(CourseAudio))
	// query k=v
	for k, v := range query {
...
// UpdateCourseAudio updates CourseAudio by Id and returns error if
// the record to be updated doesn't exist
func UpdateCourseAudioById(m *CourseAudio) (err error) {
	o := orm.NewOrm()
	v := CourseAudio{Id: m.Id}
...
// DeleteCourseAudio deletes CourseAudio by Id and returns error if
// the record to be deleted doesn't exist
func DeleteCourseAudio(id int) (err error) {
	o := orm.NewOrm()
	v := CourseAudio{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&CourseAudio{Id: id}); err == nil {
