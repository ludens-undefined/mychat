...
type CourseImgtxt struct {
	Id              int     `orm:"column:id;autoIncrement"`
	ShopId          uint    `orm:"column:shop_id"` //对应goouc_xet_business_shop表中的id
	Name            string  `orm:"column:name;size:50"` //图文标题
	Img             string  `orm:"column:img;size:255"` //图文封面
	Detail          string  `orm:"column:detail;size:100"` //图文详情路径
	Intro           string  `orm:"column:intro;size:100"` //试看内容
	...
	GoodTypeIds     string  `orm:"column:good_type_ids;size:255"` //商品可以有多个分组，商品分组id,对应goouc_xet_cource_good_type表中的id
...
func (t *CourseImgtxt) TableName() string {
	return "goouc_xmf_course_imgtxt"
}

func init() {
	orm.RegisterModel(new(CourseImgtxt))
}

// AddCourseImgtxt insert a new CourseImgtxt into database and returns
// last inserted Id on success.
func AddCourseImgtxt(m *CourseImgtxt) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetCourseImgtxtById retrieves CourseImgtxt by Id. Returns error if
// Id doesn't exist
func GetCourseImgtxtById(id int) (v *CourseImgtxt, err error) {
	o := orm.NewOrm()
	v = &CourseImgtxt{Id: id}
...
// GetAllCourseImgtxt retrieves all CourseImgtxt matches certain condition. Returns empty list if
// no records exist
func GetAllCourseImgtxt(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(CourseImgtxt))
	// query k=v
	for k, v := range query {
...
// UpdateCourseImgtxt updates CourseImgtxt by Id and returns error if
// the record to be updated doesn't exist
func UpdateCourseImgtxtById(m *CourseImgtxt) (err error) {
	o := orm.NewOrm()
	v := CourseImgtxt{Id: m.Id}
...
// DeleteCourseImgtxt deletes CourseImgtxt by Id and returns error if
// the record to be deleted doesn't exist
func DeleteCourseImgtxt(id int) (err error) {
	o := orm.NewOrm()
	v := CourseImgtxt{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&CourseImgtxt{Id: id}); err == nil {
