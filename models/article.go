...
type Article struct {
	...
	Description string `orm:"column:description;size:2000"`
	Content     string `orm:"column:content"`
	Status      int8   `orm:"column:status"` //状态 1显示 2隐藏
	IsHot       int8   `orm:"column:is_hot"` //状态 1正常 2推荐
	IsWx        int8   `orm:"column:is_wx"` //状态 1微信连接 2文本
	Click       int    `orm:"column:click"` //点击量
	Praise      int    `orm:"column:praise"` //点赞
	Sort        int    `orm:"column:sort"` //排序
	Img         string `orm:"column:img;size:2000"`
	IsDelete    int8   `orm:"column:is_delete"` //状态 1显示 2隐藏
}

func (t *Article) TableName() string {
	return "goouc_xmf_article"
}
