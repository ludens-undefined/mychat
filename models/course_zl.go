...
func (t *CourseZl) TableName() string {
	return "goouc_xmf_course_zl"
}

func init() {
	orm.RegisterModel(new(CourseZl))
}

// AddCourseZl insert a new CourseZl into database and returns
// last inserted Id on success.
func AddCourseZl(m *CourseZl) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetCourseZlById retrieves CourseZl by Id. Returns error if
// Id doesn't exist
func GetCourseZlById(id int) (v *CourseZl, err error) {
	o := orm.NewOrm()
	v = &CourseZl{Id: id}
...
// GetAllCourseZl retrieves all CourseZl matches certain condition. Returns empty list if
// no records exist
func GetAllCourseZl(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(CourseZl))
	// query k=v
	for k, v := range query {
...
// UpdateCourseZl updates CourseZl by Id and returns error if
// the record to be updated doesn't exist
func UpdateCourseZlById(m *CourseZl) (err error) {
	o := orm.NewOrm()
	v := CourseZl{Id: m.Id}
...
// DeleteCourseZl deletes CourseZl by Id and returns error if
// the record to be deleted doesn't exist
func DeleteCourseZl(id int) (err error) {
	o := orm.NewOrm()
	v := CourseZl{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&CourseZl{Id: id}); err == nil {
