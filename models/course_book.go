...
type CourseBook struct {
	...
	File            string  `orm:"column:file;size:255"` //电子书
	Size            string  `orm:"column:size;size:15"` //文件大小
	Name            string  `orm:"column:name;size:50"` //书名
	Author          string  `orm:"column:author;size:50"` //作者
	Img             string  `orm:"column:img;size:255"` //封面
	Brief           string  `orm:"column:brief;size:100"` //简介
	IsShidu         uint8   `orm:"column:is_shidu"` //1支持试读 2不支持试读
	Chapter         uint16  `orm:"column:chapter"` //对应is_shidu为1时，支持试读的章数
...
func (t *CourseBook) TableName() string {
	return "goouc_xmf_course_book"
}

func init() {
	orm.RegisterModel(new(CourseBook))
}

// AddCourseBook insert a new CourseBook into database and returns
// last inserted Id on success.
func AddCourseBook(m *CourseBook) (id int64, err error) {
	o := orm.NewOrm()
	id, err = o.Insert(m)
	return
}

// GetCourseBookById retrieves CourseBook by Id. Returns error if
// Id doesn't exist
func GetCourseBookById(id int) (v *CourseBook, err error) {
	o := orm.NewOrm()
	v = &CourseBook{Id: id}
...
// GetAllCourseBook retrieves all CourseBook matches certain condition. Returns empty list if
// no records exist
func GetAllCourseBook(query map[string]string, fields []string, sortby []string, order []string,
	offset int64, limit int64) (ml []interface{}, err error) {
	o := orm.NewOrm()
	qs := o.QueryTable(new(CourseBook))
	// query k=v
	for k, v := range query {
...
// UpdateCourseBook updates CourseBook by Id and returns error if
// the record to be updated doesn't exist
func UpdateCourseBookById(m *CourseBook) (err error) {
	o := orm.NewOrm()
	v := CourseBook{Id: m.Id}
...
// DeleteCourseBook deletes CourseBook by Id and returns error if
// the record to be deleted doesn't exist
func DeleteCourseBook(id int) (err error) {
	o := orm.NewOrm()
	v := CourseBook{Id: id}
	// ascertain id exists in the database
	if err = o.Read(&v); err == nil {
		var num int64
		if num, err = o.Delete(&CourseBook{Id: id}); err == nil {
